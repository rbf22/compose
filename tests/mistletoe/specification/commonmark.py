...
def load_tests(specfile):
    with open(specfile, encoding='utf-8') as fin:
...
def locate_section(section, tests):
    ...
    raise RuntimeError(f"Section '{section}' not found, aborting.")
...
def print_failure_in_sections(results):
    ...
    for result in results:
        if section != result[1]:
            if failed:
                section_str = f"Failed in section '{section}':"
                result_str = f"{failed:>3} / {total:>3}"
                print(f'{section_str:70} {result_str}')
            section = result[1]
            failed = 0
            total = 0
        if not result[0]:
