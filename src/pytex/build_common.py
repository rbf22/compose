...
try:
    from .symbols_data_generated import ligatures as _LIGATURES
...
_FONT_MAPPING = {
    ...
    "bb": "AMS-Regular",  # Blackboard bold often in AMS
    "sans-serif": "SansSerif-Regular",
    "typewriter": "Typewriter-Regular",
    "caligraphic": "Caligraphic-Regular",
}
...
def lookup_symbol(
    ...
    # Strip trailing whitespace from control words (TeX behavior)
    lookup_value = value.rstrip()
    
    symbol_def = SYMBOLS.get(mode, {}).get(lookup_value, {})
    replace = symbol_def.get("replace")
    if replace:
        value = replace

    defined_font = symbol_def.get("font")
    if defined_font and defined_font in _FONT_MAPPING:
        font_name = _FONT_MAPPING[defined_font]

    metrics = get_character_metrics(value, font_name, mode)
    return {
        "value": value,
        "metrics": cast(SymbolMetrics | None, metrics),
    }
...
def size_element_from_children(elem: DomNode | DocumentFragment) -> None:
...
def make_span(
    classes: list[str] | None = None,
    children: Sequence[DomNode] | None = None,
    options: Options | None = None,
...
def make_line_span(class_name: str, options: Options, thickness: float | None = None) -> DomSpan:
    """Create line span."""
