...
def _parse_value(fragment: str, index: int) -> tuple[int | tuple[int, ...], int]:
...
def _normalize_source(raw: int | tuple[int, ...]) -> GlyphSource:
...
def _normalize_target(raw: int | tuple[int, ...]) -> GlyphTarget:
    if isinstance(raw, int):
        return raw
    if len(raw) != 3:
