...
def get_variant(group: SymbolParseNode, options: Options) -> FontVariant | None:
...
def is_number_punctuation(group: MathNode | None) -> bool:
    """Check if group is number punctuation (dot or comma)."""
    if not group:
        return False

    if (group.type == 'mi' and len(group.children) == 1 and
        isinstance(group.children[0], TextNode) and group.children[0].text == '.') or (group.type == 'mo' and len(group.children) == 1 and
    ...
    for i, expr in enumerate(expression):
        ...
        if isinstance(group, MathNode) and isinstance(last_group, MathNode):
            ...
            if (group.type == 'mtext' and last_group.type == 'mtext' and
                group.get_attribute('mathvariant') == last_group.get_attribute('mathvariant')) or group.type == 'mn' and last_group.type == 'mn' or is_number_punctuation(group) and last_group.type == 'mn':
...
def build_group(group: AnyParseNode | None, options: Options) -> MathNode:
