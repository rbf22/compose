...
def padded_node(group: MathNode | None = None) -> MathNode:
...
def _xarrow_html_builder(group: ParseNode, options: Options) -> Any:
    """Build HTML for extensible arrows."""
    from .. import build_html as html
    from .. import stretchy

    xarrow_group = cast("XArrowParseNode", group)
    style = options.style

    # Build upper group
    new_options = options.having_style(style.sup())
    upper_group = wrap_fragment(
        html.build_group(xarrow_group["body"], new_options, options), options
    )

    arrow_prefix = "x" if xarrow_group["label"].startswith("\\x") else "cd"
    upper_group.classes.append(f"{arrow_prefix}-arrow-pad")

    # Build lower group if present
    lower_group = None
    if xarrow_group.get("below"):
        new_options = options.having_style(style.sub())
        lower_group = wrap_fragment(
            html.build_group(xarrow_group["below"], new_options, options), options
        )
        lower_group.classes.append(f"{arrow_prefix}-arrow-pad")

    # Create the arrow body
    arrow_body = stretchy.svg_span(xarrow_group, options)
    ...
    if (upper_group.depth > 0.25 or xarrow_group["label"] == "\\xleftequilibrium"):
...
def _xarrow_mathml_builder(group: ParseNode, options: Options) -> MathNode:
    ...
    xarrow_group = cast("XArrowParseNode", group)
    arrow_node = stretchy.math_ml_node(xarrow_group["label"])
    # Set minimum size
    min_size = "1.75em" if xarrow_group["label"].startswith("\\x") else "3.0em"
    arrow_node.set_attribute("minsize", min_size)

    if xarrow_group.get("body"):
        upper_node = padded_node(mml.build_group(xarrow_group["body"], options))
        if xarrow_group.get("below"):
            lower_node = padded_node(mml.build_group(xarrow_group["below"], options))
            node = MathNode("munderover", [arrow_node, lower_node, upper_node])
        else:
            node = MathNode("mover", [arrow_node, upper_node])
    elif xarrow_group.get("below"):
        lower_node = padded_node(mml.build_group(xarrow_group["below"], options))
