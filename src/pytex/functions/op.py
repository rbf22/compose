...
def html_builder(group: ParseNode, options: Options) -> DomSpan:
    ...
    sup_group: AnyParseNode | None = None
    sub_group: AnyParseNode | None = None
    op_group_map: Mapping[str, Any] = op_group
    has_limits = False

    if op_group.get("type") == "supsub":
        sup_group = cast(AnyParseNode | None, op_group.get("sup"))
        sub_group = cast(AnyParseNode | None, op_group.get("sub"))
...
try:
    ...
    def assembleSupSub(
        base: SymbolNode | DomSpan,
        sup_group: AnyParseNode | None,
        sub_group: AnyParseNode | None,
        options: Options,
        style: Style,
        slant: float,
        base_shift: float,
