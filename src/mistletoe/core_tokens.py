...
class Delimiter:
    ...
    def __repr__(self):
        if not self.type.startswith(('*', '_')):
            return f'<Delimiter type={repr(self.type)} active={self.active}>'
        return f'<Delimiter type={repr(self.type)} active={self.active} open={self.open} close={self.close}>'
...
class MatchObj:
    ...
    def __repr__(self):
        return f'<MatchObj fields={self.fields} start={self._start} end={self._end}>'
