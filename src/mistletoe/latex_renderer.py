...
class LaTeXRenderer(BaseRenderer):
    ...
    def render_strong(self, token):
        return f'\\textbf{{{self.render_inner(token)}}}'

    def render_emphasis(self, token):
        return f'\\textit{{{self.render_inner(token)}}}'
    ...
    def render_strikethrough(self, token):
        ...
        return f'\\sout{{{self.render_inner(token)}}}'

    def render_image(self, token):
        self.packages['graphicx'] = []
        return f'\n\\includegraphics{{{token.src}}}\n'
    ...
    def render_auto_link(self, token):
        self.packages['hyperref'] = []
        return f'\\url{{{self.escape_url(token.target)}}}'
