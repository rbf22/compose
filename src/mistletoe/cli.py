...
version_str = f'mistletoe [version {mistletoe.__version__}]'
...
def _import(arg):
    ...
    try:
        ...
        sys.exit(f'[error] cannot import module "{path}".')
    except AttributeError:
        sys.exit(f'[error] cannot find renderer "{cls_name}" from module "{path}".')
...
def _print_heading(renderer):
    ...
    if renderer is not mistletoe.HtmlRenderer:
        print(f'Using renderer: {renderer.__name__}')
