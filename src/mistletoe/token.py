...
def _short_repr(value):
    ...
    if isinstance(value, str):
        ...
        threshold = 30
        if chars > threshold:
            return f"{value[:threshold]!r}...+{chars - threshold}"
    return repr(value)
    ...
    * `span_token.SpanToken`, for all span-level (or inline-level) tokens.
    ...
    def __repr__(self):
        ...
        for attrname in self.repr_attributes:
            ...
            output += f" {attrname}={_short_repr(attrvalue)}"
        output += f" at {id(self):#x}>"
        return output
    ...
    @property
    def children(self) -> Iterable['Token'] | None:
        """
