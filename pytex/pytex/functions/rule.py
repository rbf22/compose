...
if TYPE_CHECKING:
    ...
    from ..parse_node import ParseNode, RuleParseNode, SizeParseNode
...
def html_builder(group: ParseNode, options: "Options") -> Any:
    ...
    shift_measure = rule_group.get("shift")
    shift = calculate_size(shift_measure, options) if shift_measure else 0.0
...
def mathml_builder(group: ParseNode, options: "Options") -> MathNode:
    ...
    height = calculate_size(rule_group["height"], options)
    shift_measure = rule_group.get("shift")
    shift = calculate_size(shift_measure, options) if shift_measure is not None else 0.0
...
def _rule_handler(context: Dict[str, Any], args: List[Any], opt_args: List[Optional[Any]]) -> Dict[str, Any]:
    """Handler for \rule command."""
    width_node = cast("SizeParseNode", assert_node_type(args[0], "size"))
    height_node = cast("SizeParseNode", assert_node_type(args[1], "size"))
    shift_value = None
    if opt_args and opt_args[0]:
        shift_node = cast("SizeParseNode", assert_node_type(opt_args[0], "size"))
        shift_value = shift_node["value"]
