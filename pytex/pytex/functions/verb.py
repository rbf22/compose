...
def make_verb(group: ParseNode) -> str:
    ...
    return group["body"].replace(' ', '\u2423' if group.get("star") else '\xA0')
...
def _verb_handler(context, args, opt_args):
...
def _verb_html_builder(group: ParseNode, options: Options):
    """Build HTML for verb command."""
    text = make_verb(group)
    body = []
    ...
    for c in text:
        if c == '~':
            c = '\\textasciitilde'
        body.append(make_symbol(c, "Typewriter-Regular", group["mode"], new_options,
                              ["mord", "texttt"]))
