...
def html_builder(group: ParseNode, options: Options):
    ...
    inner = html.build_group(group["body"], options.having_cramped_style())
    ...
    img = sqrt_result["span"]
    rule_width = sqrt_result["ruleWidth"]
    advance_width = sqrt_result["advanceWidth"]
    ...
    if not group.get("index"):
        return make_span(["mord", "sqrt"], [body], options)
    else:
        # Handle optional root index
        new_options = options.having_style(Style.SCRIPTSCRIPT)
        rootm = html.build_group(group["index"], new_options, options)
