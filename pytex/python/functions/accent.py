...
def html_builder(group: ParseNode, options: Options):
    ...
    if group.get("type") == "supsub":
        ...
        accent_group = group["base"]  # The accent is in the base
    ...
    must_shift = group.get("isShifty", False) and is_character_box(base)
    ...
    # Build the accent
    if not group.get("isStretchy", False):
        # Non-stretchy accent
        if group["label"] == "\\vec":
        ...
        accent_full = group["label"] == "\\textcircled"
        if accent_full:
        ...
        # Stretchy accent
        accent_body = svg_span(group, options)
...
def mathml_builder(group: ParseNode, options: Options) -> MathNode:
    ...
    if group.get("isStretchy", False):
        accent_node = math_ml_node(group["label"])
    else:
        accent_node = MathNode("mo", [mml.make_text(group["label"], group["mode"])])

    node = MathNode("mover", [
        mml.build_group(group["base"], options),
        accent_node
    ])
