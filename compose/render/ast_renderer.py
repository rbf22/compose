...
class HTMLRenderer:
    ...
    def render(self, document: Document, config: Dict[str, Any] = None) -> str:
        """Render document to HTML."""
        config = config or {}
        html_parts = []

        # Add HTML5 doctype and head
        html_parts.append('<!DOCTYPE html>')
        html_parts.append('<html lang="en">')
        html_parts.append('<head>')
        html_parts.append('    <meta charset="UTF-8">')
        html_parts.append('    <meta name="viewport" content="width=device-width, initial-scale=1.0">')
        html_parts.append(f'    <title>{config.get("title", "Compose Document")}</title>')

        # Add typography CSS if enabled
        if config.get('features', {}).get('advanced_typography', True):
            from .typography_engine import TypographyEngine
            typography_engine = TypographyEngine()
            preset = config.get('typography', {}).get('preset')
            typography_css = typography_engine.get_css_styles(preset)
            html_parts.append('    <style>')
            html_parts.append(typography_css)
            html_parts.append('    </style>')
        ...
        # Add typography class if preset is used
        body_class = ""
        if config.get('typography', {}).get('preset'):
            body_class = f' class="typography-{config["typography"]["preset"]}"'
        html_parts.append(f'<div{body_class}>')
        ...
        for block in document.blocks:
            html_block = self._render_block(block)
    ...
    <div class="math-placeholder">
        ...
        <br><small>(Math rendering will be replaced with images)</small>
            ...
            # Render mermaid diagram using diagram renderer
            from .diagram_renderer import render_mermaid_block
            return render_mermaid_block(block.content)
